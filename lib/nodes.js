...
const smartProcessing = {
    ...
    processPlains(node, allowConversion = true) {
        ...
        if (wordCount > 1 || bannedWords.includes(node.content.text.toLowerCase())) {
            if (titleRegex.test(node.content.text) && allowConversion) {
                return this.processCustomTitle(node);
            }

            // node text starts with a bullet
            if (/^[*\->→$]/.test(node.content.text) && allowConversion) {
                return {
                    type: 'listItem',
                    bold: node.bold,
                    content: {
            ...
            return allowConversion ? this.processTitle(node, HeadingLevel.HEADING_3) : node;
        }
    },
    ...
    pushNodes(nodes = []) {
        ...
        for (const node of nodes) {
            ...
            if (isEmpty) continue;
            if (ignoreRegex.test(node.content.text)) continue;

            if (textRegex.test(node.content.text)) {
                result.push(this.processPlains({
                    type: 'plain',
                    bold: node.bold,
                    content: {
                        text: node.content.text.replace(textRegex, ''),
                    },
                }, false));
